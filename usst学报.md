...
fixed:
第一个方向是基于背景建模原理，文献[3]融合局部二值模式(LBSP)背景建模和ViBe算法，基于背景差值和背景模型更新原理提出一种改进LOBSTER算法提高背景模型对动态背景的适应能力。。第二个方向基于提取特征信息训练算法，文献[4]输入大量跌倒姿态数据训练YOLOv4初步框选目标后使用自顶而下的Alphapose骨骼姿态识别框架提取关节点信息，作为人体目标行为分析的动态特征变化数据。第三个方向基于深度学习算法，文献[5]使用YOLOv3检测目标后建立时空三维卷积神经网络根据视频帧识别视频动态前景目标。文献[6]提出基于生成对抗网络（GAN）结构模型，基于视频帧序列捕捉运动中时空变化属性来识别提取视频运动目标信息特征实现动态目标识别。
...
### 算法耗时分析 表1
本文算法HFID - LOBSTER算法 - 基于骨骼点（特征提取方法）Alphapose - CNN

本文选取动态目标前景提取算法三个方向算法与HFID算法比较在室内监控视频场景下动态目标提取总耗时，对比LOBSTER算法[3]、Alphapose框架[4]、CNN算法[5]和HFID算法的总运行时间如表1所示

由表1看出图像分辨率越大算法耗时越长。HFID算法与背景差值建模LOBSTER算法总体耗时均低于1ms,相较于深度学习方法算力要求较低。骨骼关节点框架Alphapose框架和CNN算法需要对输入数据进行预标注和正常非正常数据分类，提取动态前景目标运行总耗时更久。基于深度学习的视频动态前景目标提取算法的准确率达到75%以上需要更多算力和时间进行调参训练，因此HFID比基于深度学习的视频动态前景目标提取算法应用性更强，时间资源更小。
...
从表2中看出，基于背景差值建模的经典帧差法和LOBSTER算法受室内光影干扰和抖动噪声干扰导致算法准确率和召回率均低于80%，HFID的综合评价指标比经典帧差法和LOBSTER分别提高46.5%和19.1%，弥补改进帧差法在复杂场景下动态前景目标提取鬼影问题。基于Alphapose骨骼关节点框架的动态目标提取算法准确率达80.5%而召回率为75.4%，相比正常姿态数据跌倒数据量较少导致框架训练过拟合导致该算法综合评价指标低于HFID算法13.1%。基于深度学习算法在调参训练后呈现良好的前景目标提取准确率，分别为90.9%和91.3%，但依旧存在由于数据量无法满足训练需求导致过拟合问题，HFID的综合评价指标为90.9%，比CNN和GAN动态目标前景提取算法分别增加2.7%和1.1%。从上述动态前景目标提取算法性能结果证明本文提出的HFID算法在室内单目视频场景下动态前景目标提取准确率达92.2%，优于类似方向原理的经典帧差法和LOBSTER算法，算法耗时和综合性指标优于基于深度学习的动态前景目标提取算法，并且比基于Alphapose关节特征点识别框架更轻量级。因此，HFID算法对室内单目监控视频动态前景提取具有有效性，准确率和召回率相较于其他算法有所提升
